...
from transformer_utils import PositionEmbedding
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, inputs, self_attention_mask=None):
        ...
        # First normalize the input with the LayerNorm
        # initialized in the __init__ function (self.norm)
        norm_inputs = self.norm(inputs)

        # Apply the self-attention with the normalized input,
        # use the self_attention mask as the optional mask parameter.
        attn = self.self_attention((norm_inputs, norm_inputs), mask=self_attention_mask)

        # Apply the residual connection. res_attn should sum the
        # attention output and the original, non-normalized inputs
    ...
    def forward(self, target_input, encoder_output, encoder_mask=None, decoder_mask=None, mask_future=False,
        ...
        for decoder in self.decoding_stack:
            decoder_output = decoder(decoder_output, encoder_outputs=encoder_output,
                                     self_attention_mask=self_attention_mask, cross_attention_mask=cross_attention_mask)
...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        input_embedding = TransformerInputEmbedding(d_model, vocab_size,
                                                    dropout)  # , embedding_initializer=embedding_initializer
    ...
    def forward(self, source_sequence, target_sequence, encoder_mask, decoder_mask, mask_future=True,
        ...
        # Using the self.encoder, encode the source_sequence,
        # and provide the encoder_mask variable as the optional mask.
